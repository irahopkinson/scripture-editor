...
import {
  ...
  CharNode,
  SerializedCharNode,
} from "../nodes/scripture/usj/CharNode";
import {
...
function getTextContent(markers: MarkerContent[] | undefined): string {
  if (!markers || markers.length !== 1 || typeof markers[0] !== "string")
    return "";
...
function createNote<TLogger extends LoggerBasic>(
  ...
  const previewText = elementNodes
    .reduce(
      (text, node) =>
        text +
        (node.type === "char" ? ` ${(node as SerializedTextNode).text}` : ""),
      ""
    )
    .trim();
  const node = { ...marker };
      ...
      switch (element) {
        ...
        case "note":
          lexicalNode = createNote(
            style,
            marker,
            recurseNodes(marker.content, logger),
            logger
          );
          addNode(lexicalNode, elementNodes);
          break;
...
function insertImpliedParasRecurse(
  ...
  const bookNodeIndex = nodes.findIndex(
    (node) => node.type === BookNode.getType()
  );
  if (bookNodeIndex >= 0) {
    const nodesBefore = insertImpliedParasRecurse(
      nodes.slice(0, bookNodeIndex)
    );
    const bookNode = createImpliedPara([nodes[bookNodeIndex]]);
    const nodesAfter = insertImpliedParasRecurse(
      nodes.slice(bookNodeIndex + 1)
    );
...
export function loadEditorState<TLogger extends LoggerBasic>(
  ...
  if (usj) {
    if (usj.type !== USJ_TYPE)
      logger?.warn(
        `This USJ type '${usj.type}' didn't match the expected type '${USJ_TYPE}'.`
      );
    if (usj.version !== USJ_VERSION)
      logger?.warn(
        `This USJ version '${usj.version}' didn't match the expected version '${USJ_VERSION}'.`
      );
    children = insertImpliedParasRecurse(
      recurseNodes<TLogger>(usj.content, logger)
    );
  } else {
    // 'empty' editor
    const emptyParaNode = createPara(PARA_STYLE_DEFAULT, logger);
 ...
 */
export default function UpdatePlugin<TLogger extends LoggerBasic>({
  usj,
  logger,
}: {
  usj?: Usj;
